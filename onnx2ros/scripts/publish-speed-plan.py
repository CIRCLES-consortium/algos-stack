...
def main(gpsfile, i24_geo_file, circles_planner_file, lane_control_allowable_file, myLat=None, myLong=None, nodename=None ):
    ...
    while not rospy.is_shutdown():
        ...
        control_allowable = Bool()
        lane_num_msg = Int16()
        control_allowable.data = preliminary_controls_allowed # TODO FIX temporary solution (ALWAYS setting control allowable to true)
...
#             control_allowable.data = control_allowable_value and preliminary_controls_allowed
        ...
        print("Loop result for speed planner: ", control_allowable.data, lane_num_msg.data, rospy.is_shutdown())
        sys.stdout.flush()
        r.sleep()
